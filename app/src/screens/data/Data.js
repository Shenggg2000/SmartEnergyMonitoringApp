...
const Data = () => {
  ...
  let getPropertyList = (propertiesData, pie1, pie2, pie3, pie4) => {
    let properties = [];
    propertiesData.map((item) => {
      properties.push(
        <TouchableWithoutFeedback key={item.id} onPress={() =>
          navigation.navigate(ROUTES.PROPERTY_DATA, {
  ...
  let getData = async (propertiesData) => {
    try {
      let auth_token = await authToken.getToken();
      let result = await axios.get('http://10.0.2.2:5000/api/properties/summary', { headers: { "Authorization": `Bearer ${auth_token}` } });
      let response = result.data;
      ...
      if (!response.error) {
        ...
        setWeeklyPowerPieLegend(legendCreator(weeklyPowerPie, propertiesData));
        console.log("1323");
        console.log(legendCreator(weeklyBillPie, propertiesData));
        setWeeklyBillPieLegend(legendCreator(weeklyBillPie, propertiesData));
        setMonthlyPowerPieLegend(legendCreator(monthlyPowerPie, propertiesData));
        setMonthlyBillPieLegend(legendCreator(monthlyBillPie, propertiesData));
        ...
        getPropertyList(propertiesData, weeklyPowerData, weeklyBillData, monthlyPowerData, monthlyBillData);
      }
    } catch (err) {
      return;
    }
  ...
  let legendCreator = (pie = [], propertiesData) => {
    let returnArray = [];
    pie.forEach(item => {
      if (item.name != 0) {
        let property = propertiesData.find(obj => obj.id == item.name)
        returnArray.push(
          <View style={styles.singlePieChartLegend} key={property.id}>
            <View style={[styles.singlePieChartLegendIcon, { backgroundColor: item.color }]}></View>
            <Text style={[TYPOGRAPHY.h6, FONT_COLORS.secondary, MARGIN.ml8]}>{property.property_name ? property.property_name : ""}</Text>
          </View>
        )
      } else {
        returnArray.push(
  ...
  React.useEffect(() => {
    getProperties();
  }, [isFocused, selectedBarGraphSelection, tab]);
