...
const PropertyData = () => {
  ...
  let getRoomList = (roomsData, pie1, pie2, pie3, pie4) => {
    let rooms = []
    roomsData.map((item) => {
      rooms.push(
        <TouchableWithoutFeedback key={item.id} onPress={() =>
          navigation.navigate(ROUTES.ROOM_DATA, {
  ...
  let getData = async (roomsData) => {
    try {
      let auth_token = await authToken.getToken();
      let result = await axios.get('http://10.0.2.2:5000/api/properties/usage/' + route.params.id, { headers: { "Authorization": `Bearer ${auth_token}` } });
      let response = result.data;
      ...
      if (!response.error) {
        ...
        setWeeklyPowerPieLegend(legendCreator(weeklyPowerPie, roomsData));
        setWeeklyBillPieLegend(legendCreator(weeklyBillPie, roomsData));
        setMonthlyPowerPieLegend(legendCreator(monthlyPowerPie, roomsData));
        setMonthlyBillPieLegend(legendCreator(monthlyBillPie, roomsData));
        ...
        getRoomList(roomsData, weeklyPowerData, weeklyBillData, monthlyPowerData, monthlyBillData)
      }
    } catch (err) {
      return;
    }
  ...
  let legendCreator = (pie = [], roomsData) => {
    let returnArray = [];
    pie.map(item => {
      if (item.name != 0) {
        let room = roomsData.find(obj => obj.id == item.name)
        returnArray.push(
          <View style={styles.singlePieChartLegend} key={room.id}>
            <View style={[styles.singlePieChartLegendIcon, { backgroundColor: item.color }]}></View>
            <Text style={[TYPOGRAPHY.h6, FONT_COLORS.secondary, MARGIN.ml8]}>{room.room_name ? room.room_name : ""}</Text>
          </View>
        )
      } else {
        returnArray.push(
    ...
    <SafeAreaView style={styles.main}>
      <ScrollView style={styles.container}>
        ...
        <View style={styles.allProperties}>
          <Text style={[TYPOGRAPHY.h4, FONT_COLORS.secondary, MARGIN.mb12]}>All Rooms</Text>
          {rooms}
        </View>
      </ScrollView>
    </SafeAreaView>
